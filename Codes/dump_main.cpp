...
void Decompress(string sname,string dname);
...
int main() {
	...
	do {
		...
		//往headfile.bin里写32个字节，同时新建一个子文件
		for(i=0;i<f_num;i++)
		{
			_write(ldf,c_head[i],28);
			_write(ldf,c_head2[i],4);

			ldf2=_open("temp.bin",O_RDWR|O_BINARY|O_CREAT,S_IREAD|S_IWRITE);
			...
			f_name2=string((char*)c_head[i]);
			f_name2=f_name+"//"+f_name2;

			Decompress("temp.bin",f_name2);
...
void Decompress(string sname,string dname)
{
	...
	if(dpath.substr(dpath.length() - 5, 5) != "ccmes") {
		...
		_chdir(CT_PATH);
		odr = "CrystalTile2/ -d " + spath + " -o " + dpath;
		system(odr.c_str());
		_chdir(sc_path);
	}
